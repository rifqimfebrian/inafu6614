...
summary_diff2$coefficient[2,4]

diff2 <- lm(UHI_summer_day ~ shareblack, data = uhindvitract, weights = total)
summary(diff2) #get summary of the model
coeftest(diff2, vcov = vcovHC(diff2, type="HC1")) #get robust SEs
summary_diff2 <- summary(diff2)
summary_diff2$coefficient[2,1]
summary_diff3$coefficient[2,4]

diff3 <- lm(UHI_summer_day ~ sharefemale, data = uhindvitract, weights = total)
summary(diff3) #get summary of the model
coeftest(diff3, vcov = vcovHC(diff3, type="HC1")) #get robust SEs
summary_diff3 <- summary(diff3)
summary_diff3$coefficient[2,1]
summary_diff3$coefficient[2,4]

diff4 <- lm(UHI_summer_day ~ shareover64, data = uhindvitract, weights = total)
summary(diff4) #get summary of the model
coeftest(diff4, vcov = vcovHC(diff4, type="HC1")) #get robust SEs
summary_diff4 <- summary(diff4)
summary_diff4$coefficient[2,1]
summary_diff4$coefficient[2,4]
...
###############################################################################
#DATA ANALYSIS - Understanding the strength of correlation between the X variables
...
cat("Corresponding p-value = ", anova(model)$'Pr(>F)'[1])
#which indicates that the overall regression model is significant. 
#Also, the predictor variables highpov, shareblack, and shareover64 are statistically significant 
#at the 0.05 significance level, while sharefemale is not.
...
# result: The VIF values for all variables are around 1, which are not showing any sign of multicollinearity.
...
summary(ols_b_l)
ols_b_l_robSE <- coeftest(ols_b_l, vcov = vcovHC(ols_b_l, type = "HC1"))
...
summary(ols_nb_l)
ols_nb_l_robSE <- coeftest(ols_nb_l, vcov = vcovHC(ols_nb_l, type = "HC1"))

#shareblack == 1: quadratic model with station observations
ols_b_q <- lm(UHI_summer_day ~ povt_rate + I(povt_rate^2), 
              data = ct_black, weights = total)
summary(ols_b_q)
ols_b_q_robSE <- coeftest(ols_b_q, vcov = vcovHC(ols_b_q, type = "HC1"))

#shareblack == 0: quadratic model with station observations
ols_nb_q <- lm(UHI_summer_day ~ povt_rate + I(povt_rate^2), 
               data = ct_nonblack, weights = total)
summary(ols_nb_q)
ols_nb_q_robSE <- coeftest(ols_nb_q, vcov = vcovHC(ols_nb_q, type = "HC1"))
...
linear.3 <- lm(UHI_summer_day ~ highpov + shareblack + highpov*shareblack,
               data = uhindvitract, weights = total)
my_models <- list(linear.1, linear.2, linear.3)

summary(linear.1)
summary(linear.2)
summary(linear.3)

anova(linear.1)$'Pr(>F)'[1]
anova(linear.2)$'Pr(>F)'[1]
anova(linear.3)$'Pr(>F)'[1]
